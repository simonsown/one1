...
function SimpleMarkdown({ text }) {
    const html = (text || '')
        .replace(/^## (.+)$/gm, '<h2 style="color:#289cf9;margin:16px 0 8px">$1</h2>')
        .replace(/^# (.+)$/gm, '<h1 style="color:#289cf9;margin:20px 0 10px">$1</h1>')
...
export default function LessonEditorPage({ params }) {
    ...
    if (!lesson) return (
        <div style={{ background: '#050a14', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
            ...
            <Link href="/teacher/lessons" style={{ color: '#289cf9' }}>← Quay lại</Link>
        </div>
    );

    const inputStyle = { width: '100%', background: '#0a0f1a', border: '1px solid #e7e7e7', borderRadius: '12px', padding: '14px', color: 'white', fontSize: '15px', outline: 'none', boxSizing: 'border-box' };
            ...
            <header style={{ padding: '16px 32px', background: '#0a0f1a', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        <input
                            ...
                            style={{ background: 'transparent', border: 'none', borderBottom: '2px solid #289cf9', color: 'white', fontSize: '18px', fontWeight: 800, width: '420px', outline: 'none', paddingBottom: '4px' }}
                        />
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    <button onClick={handleSaveLesson} disabled={saving}
                        style={{ background: '#289cf9', color: '#050a14', border: 'none', padding: '10px 28px', borderRadius: '10px', fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px' }}>
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                ...
                <div style={{ width: '340px', minWidth: '340px', background: '#0a0f1a', borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                    ...
                    <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <div style={{ width: '100%', height: '140px', background: '#020617', borderRadius: '10px', overflow: 'hidden', marginBottom: '10px', border: '1px dashed #e7e7e7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {lesson.thumbnail_url
                                ? <img src={lesson.thumbnail_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} />
                                : <Upload size={28} color="#e7e7e7" />}
                        </div>
                        <input placeholder="Dán URL ảnh thumbnail..." value={lesson.thumbnail_url || ''}
                            ...
                            style={{ width: '100%', background: '#050a14', border: '1px solid #e7e7e7', borderRadius: '8px', padding: '8px 12px', color: 'white', fontSize: '13px', boxSizing: 'border-box' }} />
                        <textarea placeholder="Mô tả bài học..." value={lesson.description || ''}
                            onChange={(e) => setLesson({ ...lesson, description: e.target.value })}
                            style={{ width: '100%', background: '#050a14', border: '1px solid #e7e7e7', borderRadius: '8px', padding: '8px 12px', color: 'white', fontSize: '13px', boxSizing: 'border-box', marginTop: '8px', height: '72px', resize: 'none' }} />
                        ...
                        <div style={{ marginTop: '14px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                <button onClick={() => setLesson({ ...lesson, category: 'textbook' })} style={{
                                    padding: '10px 8px', borderRadius: '10px', border: `2px solid ${(lesson.category || 'extended') === 'textbook' ? '#3b82f6' : '#e7e7e7'}`,
                                ...
                                <button onClick={() => setLesson({ ...lesson, category: 'extended' })} style={{
                                    padding: '10px 8px', borderRadius: '10px', border: `2px solid ${(lesson.category || 'extended') === 'extended' ? '#f59e0b' : '#e7e7e7'}`,
                        ...
                        <div style={{ marginTop: '20px', padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <p style={{ fontSize: '11px', color: '#289cf9', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px' }}>Nguồn tài liệu</p>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <div>
                                    <label style={{ fontSize: '10px', color: '#4b5563', display: 'block', marginBottom: '4px' }}>Tên nguồn (VD: Wikipedia)</label>
                                    <input placeholder="Nhập tên nguồn..." value={lesson.source_name || ''}
                                        onChange={(e) => setLesson({ ...lesson, source_name: e.target.value })}
                                        style={{ width: '100%', background: '#050a14', border: '1px solid #e7e7e7', borderRadius: '8px', padding: '8px 12px', color: 'white', fontSize: '13px', boxSizing: 'border-box' }} />
                                </div>
                                <div>
                                    <label style={{ fontSize: '10px', color: '#4b5563', display: 'block', marginBottom: '4px' }}>Link nguồn (URL)</label>
                    ...
                    <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        {['content', 'books'].map(tab => (
                            <button key={tab} onClick={() => setActiveTab(tab)} style={{
                                ...
                                color: activeTab === tab ? '#289cf9' : '#4b5563', border: 'none',
                                borderBottom: activeTab === tab ? '2px solid #289cf9' : '2px solid transparent',
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
                        {activeTab === 'content' ? (
                            <>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    {sections.map(s => (
                                        <div key={s.id} onClick={() => { if (activeSection && activeSection.id !== s.id) saveSection(activeSection); setActiveSection(s); }}
                                            style={{ padding: '10px 14px', borderRadius: '10px', background: activeSection?.id === s.id ? 'rgba(0,243,255,0.1)' : 'transparent', border: activeSection?.id === s.id ? '1px solid #289cf9' : '1px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', transition: 'all 0.2s' }}>
                                            <GripVertical size={14} color="#4b5563" />
                                            <div style={{ width: '28px', height: '28px', borderRadius: '6px', background: '#050a14', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#289cf9', flexShrink: 0 }}>
                                ...
                                <div style={{ marginTop: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                    {[['video', <Video size={13} />, '+Video'], ['text', <FileText size={13} />, '+Chữ'], ['image', <ImageIcon size={13} />, '+Ảnh'], ['pdf', <FileSearch size={13} />, '+PDF']].map(([type, icon, label]) => (
                                        <button key={type} onClick={() => addSection(type)} style={{ padding: '8px', background: 'rgba(255,255,255,0.03)', border: '1px solid #e7e7e7', borderRadius: '8px', color: '#8899a6', fontSize: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                            {icon} {label}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                {books.map(book => (
                                    <div key={book.id} style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid #e7e7e7', borderRadius: '10px', display: 'flex', gap: '10px' }}>
                                ...
                                }} style={{ width: '100%', padding: '12px', background: 'rgba(0,243,255,0.08)', color: '#289cf9', border: '1px dashed rgba(0,243,255,0.4)', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', fontSize: '14px' }}>
                                    + Thêm sách mới
                                </button>
                            </div>
                        )}
                    ...
                    {activeSection ? (
                        <div style={{ maxWidth: '820px', margin: '0 auto' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
                                <h2 style={{ margin: 0, color: '#289cf9', fontSize: '20px', fontWeight: 800 }}>✏️ Phần: {activeSection.type.toUpperCase()}</h2>
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    <button onClick={() => saveSection(activeSection)} style={{ background: 'rgba(0,243,255,0.1)', color: '#289cf9', border: '1px solid rgba(0,243,255,0.3)', padding: '8px 18px', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '13px' }}>💾 Lưu phần này</button>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                ...
                                {activeSection.type === 'text' && (
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ height: '360px', background: '#0a0f1a', border: '1px solid #e7e7e7', borderRadius: '12px', padding: '16px', overflowY: 'auto' }}>
                                ...
                                {activeSection.type === 'image' && <>
                                    ...
                                    {activeSection.content && <img src={activeSection.content} style={{ maxWidth: '100%', borderRadius: '12px', border: '1px solid #e7e7e7' }} onError={e => e.target.style.display = 'none'} />}
                                </>}
                                ...
                                {activeSection.type === 'pdf' && <>
                                    <div style={{ background: 'rgba(0,243,255,0.05)', border: '1px solid rgba(0,243,255,0.15)', borderRadius: '12px', padding: '16px', fontSize: '13px', color: '#8899a6', lineHeight: 1.6 }}>
                                        💡 <strong style={{ color: '#289cf9' }}>Hướng dẫn lấy link Drive:</strong> Vào Google Drive → Chuột phải file PDF → <em>Chia sẻ</em> → Sao chép link → Đổi <code>/view</code> thành <code>/preview</code>
                                    </div>
                                    <div>
                                    ...
                                    {getDriveEmbed(activeSection.content) && (
                                        <div style={{ width: '100%', height: '500px', background: 'white', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e7e7e7' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#e7e7e7', gap: '16px' }}>
                            <PlusCircle size={64} />
