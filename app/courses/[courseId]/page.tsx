...
export default function CourseDetailPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  if (!course) return (
    <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center text-red-500 font-bold">
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} className="flex-1 text-center lg:text-left">
               <div className="inline-flex items-center gap-2 px-4 py-1 bg-[#089e60]/10 border border-[#089e60]/20 rounded-full text-[#089e60] text-xs font-bold uppercase tracking-widest mb-6">
               ...
               <div className="flex flex-wrap gap-6 justify-center lg:justify-start">
                  <div className="flex items-center gap-2 text-slate-300">
                    <Clock size={20} className="text-[#089e60]" /> <span>4 giờ học</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-300">
                    <Layers size={20} className="text-[#089e60]" /> <span>{lessons.length} bài học</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-300">
                    <CheckCircle size={20} className="text-[#089e60]" /> <span>Cấp chứng chỉ</span>
                  </div>
               </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="lg:w-[450px] shrink-0">
               <div className="bg-[#ffffff] border border-[#e7e7e7] rounded-[40px] p-2 shadow-2xl overflow-hidden relative group">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
         <h2 className="text-3xl font-bold mb-10">Lộ trình <span className="text-[#089e60]">Bài giảng</span></h2>
         ...
         <div className="space-y-6">
            {lessons.map((lesson, idx) => (
              <motion.div 
                ...
                className="bg-[#ffffff] border border-[#e7e7e7] rounded-3xl overflow-hidden"
              >
                 <div className="p-6 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                       <div className="w-12 h-12 rounded-2xl bg-[#f8f9fa] border border-[#e7e7e7] flex items-center justify-center text-slate-500 font-black">
                          {idx + 1}
                       </div>
                       <div>
                          <h3 className="font-bold text-lg group-hover:text-[#089e60] transition-colors">{lesson.title}</h3>
                 ...
                 <div className="px-6 pb-6 pt-2 border-t border-[#e7e7e7]/50 space-y-3">
                    {lesson.lesson_sections?.map((section: any) => (
                       <div key={section.id} className="flex items-center justify-between py-2 px-4 rounded-xl hover:bg-[#f8f9fa]/50 transition-colors">
                          <div className="flex items-center gap-3">
                             {section.content_type === 'video' ? <Play size={14} className="text-[#089e60]" /> : 
