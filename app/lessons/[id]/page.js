        ...
        <div style={{ background: '#050a14', minHeight: '100vh', color: '#e0e6ed', display: 'flex' }}>
            ...
            <main style={{ flex: 1, padding: '64px', maxWidth: '900px', margin: '0 auto' }}>
                <header style={{ marginBottom: '64px' }}>
                    ...
                    {lesson.source_name && (
                        <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', color: 'rgba(255,255,255,0.4)' }}>
                            <span>Nguồn tài liệu:</span>
                            {lesson.source_url ? (
                                <a href={lesson.source_url} target="_blank" style={{ color: 'var(--brand-primary)', textDecoration: 'none', fontWeight: 600 }}>{lesson.source_name}</a>
                            ) : (
                                <span style={{ fontWeight: 600 }}>{lesson.source_name}</span>
                            )}
                        </div>
                    )}
