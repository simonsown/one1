    ...
    <div style={{ margin: 0 }}>
      <motion.header {...fadeUp(0)} style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 22px', borderRadius: '8px', border: 'none', background: s.navy, color: '#fff', fontWeight: 700, fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <motion.div {...fadeUp(0.1)} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
        {stats.map((stat, idx) => (
          <motion.div key={idx} whileHover={{ y: -3 }}
            style={{ padding: '22px 24px', display: 'flex', alignItems: 'center', gap: '16px', borderRadius: '12px', border: '1px solid #E2E8F0', background: '#fff', transition: 'box-shadow 0.2s' }}>
            <div style={{ width: '46px', height: '46px', borderRadius: '10px', background: `${stat.color}0d`, color: stat.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {stat.icon}
            </div>
            <div>
              <div style={{ fontSize: '22px', fontWeight: 800, color: s.navy, lineHeight: 1.2 }}>{stat.value}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '24px' }}>
        <section style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <motion.div {...fadeUp(0.2)}
            style={{ padding: '28px 32px', position: 'relative', overflow: 'hidden', borderRadius: '14px', background: `linear-gradient(135deg, ${s.navy} 0%, #0F2A44 100%)`, border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                <Monitor size={18} color={s.teal} />
                <span style={{ color: s.teal, fontSize: '12px', fontWeight: 700, letterSpacing: '1px' }}>PHÒNG LAB 2D</span>
              </div>
              <h2 style={{ fontSize: '18px', fontWeight: 800, marginBottom: '8px', color: '#fff' }}>Thực hành lắp ráp PC</h2>
              ...
              <Link href="/builder">
                <button style={{ padding: '10px 24px', borderRadius: '8px', border: 'none', background: s.red, color: '#fff', fontWeight: 700, fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
          ...
          <motion.div {...fadeUp(0.3)} style={{ padding: '24px 28px', borderRadius: '12px', border: '1px solid #E2E8F0', background: '#fff' }}>
            ...
            {loading ? (
                  ...
                  <motion.div key={cls.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 + idx * 0.05 }}>
                    <Link href={`/student/classes/${cls.id}`} style={{ textDecoration: 'none' }}>
                      <motion.div whileHover={{ y: -2 }}
                        style={{ padding: '16px', cursor: 'pointer', borderRadius: '10px', border: '1px solid #E2E8F0', background: '#F8FAFC', transition: 'box-shadow 0.2s' }}>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '40px 0', border: '2px dashed #E2E8F0', borderRadius: '10px' }}>
                <p style={{ color: '#64748B', marginBottom: '12px', fontSize: '13px' }}>Bạn chưa tham gia lớp học nào.</p>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={() => setShowJoinModal(true)}
                  style={{ background: 'transparent', border: `1px solid ${s.navy}`, color: s.navy, padding: '8px 20px', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px' }}>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <motion.div {...fadeUp(0.25)}
            style={{ padding: '24px', background: `linear-gradient(135deg, ${s.teal} 0%, #006064 100%)`, color: '#fff', position: 'relative', overflow: 'hidden', borderRadius: '14px' }}>
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <Link href="/builder">
                <button style={{ display: 'block', textAlign: 'center', padding: '10px', background: 'rgba(255,255,255,0.15)', color: '#fff', borderRadius: '8px', fontWeight: 700, textDecoration: 'none', fontSize: '13px', border: '1px solid rgba(255,255,255,0.2)', width: '100%', cursor: 'pointer', fontFamily: 'inherit' }}>
                  Bắt đầu trò chuyện
                </button>
              </Link>
            </div>
          ...
          <motion.div {...fadeUp(0.35)} style={{ padding: '20px', borderRadius: '12px', border: '1px solid #E2E8F0', background: '#fff' }}>
      ...
      <Link href="/builder" style={{
        ...
        boxShadow: '0 6px 20px rgba(211,47,47,0.35)', cursor: 'pointer', zIndex: 100, textDecoration: 'none',
        transition: 'transform 0.2s ease, box-shadow 0.2s ease'
      }} onMouseOver={e => { e.currentTarget.style.transform = 'scale(1.1)'; e.currentTarget.style.boxShadow = '0 8px 28px rgba(211,47,47,0.45)'; }}
         onMouseOut={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(211,47,47,0.35)'; }}>
