...
function ThemeToggle() {
  ...
  return (
    <button onClick={toggle} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 14px', width: '100%', borderRadius: '8px', background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.4)', fontWeight: 500, cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit', transition: 'all 0.2s' }}>
  ...
  const NavItem = ({ href, icon: Icon, label }: { href: string; icon: any; label: string }) => {
    ...
    return (
      <Link href={href} style={{
        display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 14px', borderRadius: '8px', textDecoration: 'none', fontSize: '13px',
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F8FAFC' }}>
      <aside style={{ width: '260px', background: `linear-gradient(180deg, #0B1A2E 0%, ${s.navy} 100%)`, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '16px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'relative' }}>
          <button onClick={() => setShowDropdown(!showDropdown)}
            style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: '8px', fontFamily: 'inherit' }}>
            <div style={{ width: '34px', height: '34px', borderRadius: '8px', background: s.red, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ flex: 1, textAlign: 'left', minWidth: 0 }}>
              ...
              <div style={{ fontSize: '10px', color: s.red, fontWeight: 600 }}>Giáo viên</div>
            </div>
          ...
          {showDropdown && (
            <>
              ...
              <div style={{ position: 'absolute', top: '100%', left: '12px', right: '12px', background: '#fff', borderRadius: '10px', border: '1px solid #E2E8F0', boxShadow: '0 10px 40px rgba(0,0,0,0.12)', zIndex: 100, padding: '8px' }}>
                <div style={{ padding: '8px 10px', borderBottom: '1px solid #F1F5F9', marginBottom: '4px' }}>
                  ...
                  <div style={{ fontSize: '10px', color: s.red, fontWeight: 600, marginTop: '4px' }}>Vai trò: Giáo viên</div>
                </div>
                <Link href="/teacher" onClick={() => setShowDropdown(false)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', borderRadius: '6px', color: s.navy, textDecoration: 'none', fontSize: '12px' }}>
                  <LayoutDashboard size={15} /> Bảng điều khiển
                </Link>
                <button onClick={async () => { setShowDropdown(false); await logout() }}
                  style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', width: '100%', borderRadius: '6px', background: 'none', border: 'none', color: s.red, cursor: 'pointer', fontSize: '12px', textAlign: 'left', fontFamily: 'inherit' }}>
        ...
        <div style={{ padding: '10px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          <button onClick={async () => { await logout() }}
            style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 14px', width: '100%', borderRadius: '8px', background: 'transparent', border: 'none', color: 'rgba(244,67,54,0.6)', fontWeight: 500, cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', minWidth: 0 }}>
        <div style={{ padding: '28px 36px', maxWidth: '1200px' }}>
          <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#94A3B8' }}>
            <Link href="/teacher" style={{ color: s.red, textDecoration: 'none', fontWeight: 500 }}>Bảng điều khiển</Link>
            <span>/</span>
